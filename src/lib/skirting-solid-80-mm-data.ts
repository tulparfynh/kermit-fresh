...
async function getPanelFromDirectory(dirName: string): Promise<Panel | null> {
    ...
    try {
        ...
        const baseImagePath = `/images/skirting-boards/solid-80-mm-skirting-board/${dirName}`;
        ...
        return {
            ...
            productImageHint: `product view for solid 80mm skirting ${dirName}`,
            applicationImageHint: `application view for solid 80mm skirting ${dirName}`,
        };
    } catch (error) {
        console.warn(`Could not process solid 80mm skirting in '${dirName}'. Ensure 'product.jpg' and 'application.jpg' exist.`, error);
        return null;
    }
}
...
export async function getSkirtingSolid80mm(): Promise<Panel[]> {
  ...
  try {
    ...
    if (!Array.isArray(productKeys)) {
        throw new Error('products.json in solid-80-mm-skirting-board is not a valid JSON array.');
    }
    ...
    if (error instanceof SyntaxError) {
        console.error("Error: Malformed JSON in 'solid-80-mm-skirting-board/products.json'.", error);
    } else if (error && (error as NodeJS.ErrnoException).code === 'ENOENT') {
        console.error("Error: 'public/images/skirting-boards/solid-80-mm-skirting-board/products.json' manifest file not found.", error);
    } else {
        console.error("Could not read or process solid 80mm skirting data from 'products.json'.", error);
    }
    return [];
  }
}
