...
async function getPanelFromDirectory(dirName: string): Promise<Panel | null> {
    ...
    try {
        ...
        const baseImagePath = `/images/skirting-boards/elite-100-mm-skirting-board/${dirName}`;
        ...
        return {
            ...
            productImageHint: `product view for elite 100mm skirting ${dirName}`,
            applicationImageHint: `application view for elite 100mm skirting ${dirName}`,
        };
    } catch (error) {
        console.warn(`Could not process elite 100mm skirting in '${dirName}'. Ensure 'product.jpg' and 'application.jpg' exist.`, error);
        return null;
    }
}
...
export async function getSkirtingElite100mm(): Promise<Panel[]> {
  ...
  try {
    ...
    if (!Array.isArray(productKeys)) {
        throw new Error('products.json in elite-100-mm-skirting-board is not a valid JSON array.');
    }
    ...
    if (error instanceof SyntaxError) {
        console.error("Error: Malformed JSON in 'elite-100-mm-skirting-board/products.json'.", error);
    } else if (error && (error as NodeJS.ErrnoException).code === 'ENOENT') {
        console.error("Error: 'public/images/skirting-boards/elite-100-mm-skirting-board/products.json' manifest file not found.", error);
    } else {
        console.error("Could not read or process elite 100mm skirting data from 'products.json'.", error);
    }
    return [];
  }
}
