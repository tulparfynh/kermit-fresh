...
async function getPanelFromDirectory(dirName: string): Promise<Panel | null> {
    ...
    try {
        ...
        const baseImagePath = `/images/spc-parquet-stone-collection/${dirName}`;
        ...
        return {
            ...
            productImageHint: `product view for stone parquet ${dirName}`,
            applicationImageHint: `application view for stone parquet ${dirName}`,
        };
    } catch (error) {
        console.warn(`Could not process stone parquet in '${dirName}'. Ensure 'product.jpg' and 'application.jpg' exist.`, error);
        return null;
    }
}
...
export async function getFloorStone(): Promise<Panel[]> {
  ...
  try {
    ...
    if (!Array.isArray(productKeys)) {
        throw new Error('products.json in spc-parquet-stone-collection is not a valid JSON array.');
    }
    ...
    if (error instanceof SyntaxError) {
        console.error("Error: Malformed JSON in 'spc-parquet-stone-collection/products.json'.", error);
    } else if (error && (error as NodeJS.ErrnoException).code === 'ENOENT') {
        console.error("Error: 'public/images/spc-parquet-stone-collection/products.json' manifest file not found.", error);
    } else {
        console.error("Could not read or process stone parquet data from 'products.json'.", error);
    }
    return [];
  }
}
